// Command Dedline-cli is a CLI + REPL for the Dedline SDK,
...
const prompt = "dedline"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "deadline registration_feature stat state"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DEDLINE_APIKEY (api key), DEDLINE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DEDLINE_APIKEY carries the API key and
	// DEDLINE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DEDLINE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DEDLINE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDedlineSDK(opts)
