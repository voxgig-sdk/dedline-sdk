...
boru-driven command-line client **and** interactive REPL for the Dedline
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dedline-cli)
make build
...
./dedline-cli list registration_feature

# 5. Override the API base URL for a single call
DEDLINE_BASE=https://api.example.com ./dedline-cli list deadline

# 6. No arguments -> interactive REPL
./dedline-cli
dedline> list deadline
dedline> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dedline-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/dedline-cli list deadline
   ```
...
Run with no arguments to open a REPL (prompt `dedline>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `deadline`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/dedline-cli`. |
...
(`./dedline-cli <expr>`) and interactively (the REPL), and expressions compose the
