...
class RegistrationFeatureEntity
  ...
  def make
    opts = @_entopts.dup
    RegistrationFeatureEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = DedlineHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [RegistrationFeature, Hash] the current RegistrationFeature data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = DedlineHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of RegistrationFeature fields)
  def match_get
  ...
  #
  # @param reqmatch [RegistrationFeatureListMatch, Hash, nil] match filter (any subset of
  #   RegistrationFeature fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<RegistrationFeature>, Array] the matching RegistrationFeature items; raises DedlineError on failure
