...
class StateEntity
  ...
  # @return [State, Hash] the loaded State; raises DedlineError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = DedlineHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<State>, Array] the matching State items; raises DedlineError on failure
