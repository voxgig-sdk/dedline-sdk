...
require_relative 'Dedline_types'
...
class DedlineSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = DedlineHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = DedlineHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, DedlineFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = DedlineHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = DedlineHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = DedlineHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => DedlineError.new(
        "#{op}_allow",
        "DedlineSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = DedlineHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = DedlineError.new(
        "graphql_error", "DedlineSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
