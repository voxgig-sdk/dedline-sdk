...
The Python SDK for the Dedline API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Deadline(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Dedline(None).load(
    {"id": "test01"}, None
)
