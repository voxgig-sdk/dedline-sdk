...
class TestRegistrationFeatureEntity:

    def test_should_create_instance(self):
        testsdk = DedlineSDK.test(None, None)
        ent = testsdk.RegistrationFeature(None)
    ...
    def test_should_stream(self):
        ...
        base = DedlineSDK.test(seed, None)
        seen = list(base.RegistrationFeature(None).stream("list", None, None))
        assert len(seen) == 3

        # Inbound: streaming active -> yields each item from the feature.
        from dedline_sdk.config import shared_config
        cfg = shared_config()
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = DedlineSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.RegistrationFeature(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _registration_feature_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "registration_feature." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DEDLINE_TEST_REGISTRATION_FEATURE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        registration_feature_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.registration_feature")))
        registration_feature_ref01_data = None
        if len(registration_feature_ref01_data_raw) > 0:
...
def _registration_feature_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/registration_feature/RegistrationFeatureTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("DEDLINE_TEST_REGISTRATION_FEATURE_ENTID"))
    ...
    if env.get("DEDLINE_TEST_LIVE") == "TRUE":
        ...
        client = DedlineSDK(helpers.to_map(merged_opts))

    _live = env.get("DEDLINE_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("DEDLINE_TEST_EXPLAIN") == "TRUE",
        "live": _live,
