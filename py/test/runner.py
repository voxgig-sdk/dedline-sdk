...
class DedlineTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DedlineTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DedlineTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DedlineTestRunner.getenv("DEDLINE_TEST_LIVE")
        override = DedlineTestRunner.getenv("DEDLINE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DedlineTestRunner.getenv(key)
        ...
        explain = DedlineTestRunner.getenv("DEDLINE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["DEDLINE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if DedlineTestRunner._test_control is not None:
            return DedlineTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DedlineTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DedlineTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DedlineTestRunner.load_test_control()
...
def load_env_local():
    DedlineTestRunner.load_env_local()


def env_override(m):
    return DedlineTestRunner.env_override(m)


def entity_data(v):
    return DedlineTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DedlineTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DedlineTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DedlineTestRunner.load_test_control()


def live_client_options():
    return DedlineTestRunner.live_client_options()


def live_delay_ms():
    return DedlineTestRunner.live_delay_ms()
