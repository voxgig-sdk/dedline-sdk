...
The Lua SDK for the Dedline API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Deadline(nil):list(nil, nil)
...
local result, err = client:Dedline(nil):load(
  { id = "test01" }, nil
)
