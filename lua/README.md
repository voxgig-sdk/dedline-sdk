...
The Lua SDK for the Dedline API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Deadline()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/dedline-sdk/releases)),
...
```
lua/
├── dedline_sdk.lua    -- Main SDK module
...
The main module (`dedline_sdk`) exports the SDK constructor
