...
local DeadlineEntity = {}
DeadlineEntity.__index = DeadlineEntity


function DeadlineEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DeadlineEntity)
  self._name = "deadline"
  self._client = client
...
function DeadlineEntity:get_name()
  return self._name
end


function DeadlineEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DeadlineEntity.new(self._client, opts)
end
...
function DeadlineEntity:mark_deleted()
  self._deleted = true
end


function DeadlineEntity:deleted()
  return true == self._deleted
end


function DeadlineEntity:data_set(args)
  if args ~= nil then
...
function DeadlineEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DeadlineEntity:match_set(args)
  if args ~= nil then
...
function DeadlineEntity:match_get()
...
function DeadlineEntity:stream(action, args, callopts)
...
function DeadlineEntity:_run_op(ctx, post_done)
  local utility = self._utility
