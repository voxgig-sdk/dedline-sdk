...
DedlineSDK._make_feature = _make_feature


function DedlineSDK.new(options)
  local self = setmetatable({}, DedlineSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function DedlineSDK:options_map()
...
function DedlineSDK:get_utility()
  return Utility.copy(self._utility)
end


function DedlineSDK:get_root_ctx()
  return self._rootctx
end


function DedlineSDK:prepare(fetchargs)
...
function DedlineSDK:direct(fetchargs)
...
-- by passing it.
function DedlineSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function DedlineSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "DedlineSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function DedlineSDK:Deadline(data)
  ...
  if data == nil then
    if self._deadline == nil then
      self._deadline = EntityMod.new(self, nil)
    end
...
function DedlineSDK:RegistrationFeature(data)
  ...
  if data == nil then
    if self._registration_feature == nil then
      self._registration_feature = EntityMod.new(self, nil)
    end
    return self._registration_feature
  end
  return EntityMod.new(self, data)
end
...
function DedlineSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = DedlineSDK.new(sdkopts)
  sdk.mode = "test"
