...
import (
	...
	omni "github.com/voxgig-sdk/dedline-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DedlineError); is {
				out := map[string]any{
					"name":    "DedlineError",
...
func omniCtx(arg any, client *sdk.DedlineSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DedlineSDK); is {
			client = live
