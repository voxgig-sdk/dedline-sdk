...
The Golang SDK for the Dedline API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.

It exposes the API as capitalised, semantic **Entities** — e.g. `client.Deadline(nil)` — each with the same small set of operations (`List`, `Load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
## Install
```bash
go get github.com/voxgig-sdk/dedline-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/dedline-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/dedline-sdk/go=../dedline-sdk/go
```
...
deadline, err := client.Deadline(nil).List(
    nil, nil,
)
if err != nil {
    panic(err)
}
fmt.Println(deadline) // the returned mock data
```
...
slice):
    ...
    deadline, err := client.Deadline(nil).List(map[string]any{/* fields */}, nil)
    if err != nil { /* handle */ }
    // deadline is the returned record
