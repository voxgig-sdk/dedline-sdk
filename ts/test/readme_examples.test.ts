...
//     (\`new DedlineSDK(...)\` / \`DedlineSDK.test(...)\` become
//     \`DedlineSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"deadline":{"test01":{"id":"test01"}},"registration_feature":{"test01":{"id":"test01"}},"stat":{"test01":{"id":"test01"}},"state":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DedlineSDK(...)\` and any
// \`DedlineSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DedlineSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DedlineSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
