...
describe('RegistrationFeatureEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'registration_feature.' + op, live)) return
    }
    ...
    const registration_feature_ref01_list = await registration_feature_ref01_ent.list(registration_feature_ref01_match)
