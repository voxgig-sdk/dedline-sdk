...
describe('RegistrationFeatureDirect', async () => {
  ...
  test('direct-list-registration_feature', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-registration_feature', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DEDLINE_TEST_REGISTRATION_FEATURE_ENTID']
