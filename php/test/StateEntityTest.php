...
class StateEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set DEDLINE_TEST_STATE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function state_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["state01", "state02", "state03", "state_abbreviation01"] as $k) {
    ...
    $entid_env_raw = getenv("DEDLINE_TEST_STATE_ENTID");
