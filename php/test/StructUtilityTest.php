...
// Dedline SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = DedlineOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(DedlineOmni::structfix($now, Struct::undef()))
                        !== json_encode(DedlineOmni::structfix($val, Struct::undef()))) {
