...
// Dedline SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class DedlineMakeError
{
    public static function call(?DedlineContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new DedlineContext([], null);
        }
        $op = $ctx->op ?? new DedlineOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new DedlineResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof DedlineError) ? $err->msg : (string)$err;
        $msg = "DedlineSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new DedlineError('', $msg, $ctx);
        ...
        if ($err instanceof DedlineError) {
