...
class DedlineNetsimFeature extends DedlineBaseFeature
{
    ...
    private function _simulate(DedlineContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(DedlineContext $ctx, array $applied): void
    {
        $client = $this->client;
