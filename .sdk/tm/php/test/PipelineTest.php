...
{
    private static function utility(): DedlineUtility
    {
        return new DedlineUtility();
    }
    ...
    private static function ctx(array $over = []): DedlineContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new DedlineContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new DedlineOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof DedlineError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new DedlineOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = DedlineMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new DedlineOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = DedlineMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new DedlineOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = DedlineMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DedlineSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = DedlineMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new DedlineResponse([]),
            'result' => new DedlineResult([]),
        ]);
        [, $err] = DedlineMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new DedlineSpec([]),
            'response' => null,
            'result' => new DedlineResult([]),
        ]);
        [, $err] = DedlineMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new DedlineSpec(['step' => 's']),
            'response' => new DedlineResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new DedlineResult([]),
        ]);
        [, $err] = DedlineMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new DedlineSpec(['step' => 's']),
            'response' => new DedlineResponse(self::resp(200, ['v' => 1])),
            'result' => new DedlineResult([]),
        ]);
        [, $err] = DedlineMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new DedlineSpec(['step' => 's']),
            'response' => new DedlineResponse(self::resp(200, ['v' => 2])),
            'result' => new DedlineResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        DedlineMakeResponse::call($ctx);
    ...
    {
        $preset = new DedlineResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new DedlineSpec([]),
            'response' => new DedlineResponse([]),
            'result' => new DedlineResult([]),
        ...
        [$got, $err] = DedlineMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new DedlineResult([])]);
        [, $err] = DedlineMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new DedlineSpec([]), 'result' => null]);
        [, $err] = DedlineMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DedlineSpec(['step' => 's']),
            'result' => new DedlineResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new DedlineOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DedlineMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DedlineSpec(['step' => 's']),
            'result' => new DedlineResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new DedlineOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DedlineMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DedlineResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new DedlineSpec([]),
            'result' => new DedlineResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = DedlineMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DedlineMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new DedlineError('boom', 'boom');
        $utility->fetcher = function (DedlineContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DedlineSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = DedlineMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (DedlineContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = DedlineMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(DedlineResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (DedlineContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DedlineSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = DedlineMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new DedlineResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new DedlineSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = DedlineMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = DedlineMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = DedlineMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new DedlineResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, DedlineDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new DedlineResult(['ok' => false])]);
        $this->expectException(DedlineError::class);
        DedlineDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new DedlineResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', DedlineMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        DedlineMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new DedlineBaseFeature();
        $b = new DedlineBaseFeature();
        DedlineFeatureAdd::call($ctx, $a);
        DedlineFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): DedlineBaseFeature
    {
        $f = new DedlineBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        DedlineFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): DedlineContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new DedlineSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DedlinePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new DedlineResponse(['headers' => null]),
            'result' => new DedlineResult([]),
        ]);
        DedlineResultHeaders::call($ctx);
