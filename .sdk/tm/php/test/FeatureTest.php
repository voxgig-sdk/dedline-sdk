...
class FtCtrl extends DedlineControl
{
...
class FtClient
{
    ...
    private DedlineUtility $utility;

    public function __construct(DedlineUtility $utility, array $options)
    {
    ...
    public function get_utility(): DedlineUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DedlineContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DedlineContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DedlineConfig::make_feature($name);
    ...
    private static function buildUrl(DedlineSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DedlineError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DedlineResponse($fetched);
        }
    ...
    private function fail(DedlineContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DedlineContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
