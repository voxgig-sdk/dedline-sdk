...
[MCP](https://modelcontextprotocol.io) server exposing the Dedline SDK as
two agent tools — `dedline_list` and `dedline_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dedline-mcp)
make build

# 2. Provide credentials via the environment
export DEDLINE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user dedline \
  -- /absolute/path/to/dedline-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./dedline-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// dedline_list: first page of records
{ "entity": "deadline" }
{ "entity": "deadline", "query": { } }

// dedline_load: one record by id
{ "entity": "stat", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dedline-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user dedline \
     -- "$PWD"/dist/*/dedline-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `dedline_list` and `dedline_load` tools now appear
   in new sessions. Ask the agent to *"list deadline using dedline"*
   and it calls `dedline_list` with `{"entity":"deadline"}`.
...
```sh
export DEDLINE_APIKEY=sk_live_xxx            # API key
export DEDLINE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `dedline_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `dedline_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DEDLINE_APIKEY` | API key sent with every request. |
| `DEDLINE_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

deadline | registration_feature | stat | state

### Smoke test via HTTP (raw JSON-RPC)

```sh
./dedline-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dedline_load","arguments":{"entity":"stat","query":{"id":1}}}}'
```
