...
type Args struct {
	Entity string         `json:"entity" jsonschema:"deadline | registration_feature | stat | state"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.DedlineSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "dedline_list",
		Description: "List records from Dedline. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "dedline_load",
		Description: "Load a single record from Dedline. " +
...
func runOp(client *sdk.DedlineSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.DedlineSDK, name string) (sdk.DedlineEntity, error) {
	switch strings.ToLower(name) {
	case "deadline":
		return client.Deadline(nil), nil
	case "registration_feature":
		return client.RegistrationFeature(nil), nil
	case "stat":
